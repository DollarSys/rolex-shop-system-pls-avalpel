...
module.exports = {
  ...
  run: async(client, message, args) => {
    ...
    if(!name1) return message.reply({embeds: [howmsg]}).then(async(msggggggggg) => {
      setTimeout(() => message.delete(), 3000)
      setTimeout(() => msggggggggg.delete(), 3000);        
    })
    ...
    message.reply({content: `ما هو نوع طلبك؟ ,<@${message.author.id}>`, embeds: [ordernew], components: [row]}).then(async (mmbtn) => {
      ...
      if(collect.customId.startsWith("cansel")) {
        mmbtn.delete();
        await message.reply({ content: `${message.author}, ❌ **تم إلغاء هذا الطلب بنجاح.**`, ephemeral: true }).then(async (msg1)=> {
       ...
       if(collect.customId.startsWith("mntgat")) {
          mmbtn.delete();
          await message.reply({ content: `${message.author}, ✅ **تم إرسال هذا الطلب إلي البائعين بنجاح.**`, ephemeral: true }).then(async (msg2)=> {
       ...
       if(collect.customId.startsWith("tsamem")) {
          mmbtn.delete();
          await message.reply({ content: `${message.author}, ✅ **تم إرسال هذا الطلب إلي البائعين بنجاح.**`, ephemeral: true }).then(async (msg3)=> {
            let channel = message.guild.channels.cache.get(config.order_room_send);
            if(!channel) return;
            channel.send({ content: `** :طلب جديد من ${message.author} - <@&${config.order_mention_role}> ** <:RolexsPin:1012969637597675600>`, embeds: [ordernew1] })
          setTimeout(() => message.delete(), 1000)
          setTimeout(() => msg3.delete(), 3000);            
        })
       }
    })
   }
