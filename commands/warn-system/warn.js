...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if(!user) return message.reply({ content: `❌ **منشن الشخص الذي تريد إعطائة تحذير.**` });
    ...
    if(warn_level == 0 || warn_level == 1) {
      user.roles.add(warn_level == 0 ? config.warn1 : config.warn2).then(() => {
        message.reply({ content: `✅ Done warn **<@${user.user.id}>**` });
      }).catch(() => {
        message.reply({ content: `❌ I can't warn **<@${user.user.id}>**` });
      });
    } else {
      user.roles.cache.filter(r => r.id != config.member_role).forEach(ro => user.roles.remove(ro).catch(() => 0));
      message.reply({ content: `✅ Done warn **<@${user.user.id}>** and remove his roles.` });
    }
  }
}
